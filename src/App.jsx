...
export default function App() {
  ...
  // page needs to touch storage.  Refreshed via "jetflix:player-settings-changed".
  ...
  useEffect(() => {
    ...
    window.electron.getAppVersion().then((version) => {
      const lastVersion = localStorage.getItem("jetflix_lastVersion");
      if (lastVersion && lastVersion !== version) {
        clearAppCaches();
      }
      localStorage.setItem("jetflix_lastVersion", version);
    });
  }, []);
  ...
  useEffect(() => {
    ...
    window.addEventListener("jetflix:library-sort-changed", handler);
    return () =>
      window.removeEventListener("jetflix:library-sort-changed", handler);
  }, []);

  // ── Re-fetch trending immediately when metadata language changes ──────────
  useEffect(() => {
    const handler = () => fetchTrending();
    window.addEventListener("jetflix:tmdb-lang-changed", handler);
    return () =>
      window.removeEventListener("jetflix:tmdb-lang-changed", handler);
  }, [fetchTrending]);

  // ── Refresh player settings (accent + subtitle lang) after save ───────────
  useEffect(() => {
    const handler = () => setPlayerSettings(readPlayerSettings());
    window.addEventListener("jetflix:player-settings-changed", handler);
    return () =>
      window.removeEventListener("jetflix:player-settings-changed", handler);
  }, []);
  ...
  useEffect(() => {
    ...
    window.addEventListener("jetflix:discord-rpc-settings-changed", handler);
    return () =>
      window.removeEventListener(
        "jetflix:discord-rpc-settings-changed",
        handler,
      );
  }, []);
    ...
    <ErrorBoundary>
      ...
      <div>
        ...
        {updateBanner && (
          ...
          >
            <span>🎉 Jetflix v{updateBanner.latest} is available!</span>
            <button
