...
export const GITHUB_REPO = releaseConfig.githubRepository || "";
...
export const UPDATE_SOURCES = GITHUB_REPO
  ? {
      github: {
    id: "github",
    label: "GitHub",
...
export const DEFAULT_UPDATE_SOURCE = Object.keys(UPDATE_SOURCES)[0] || "github";
...
export async function checkForUpdates(source = DEFAULT_UPDATE_SOURCE) {
  ...
  if (!cfg) {
    throw new Error(
      "Updates are not configured for this local build. Build with the included GitHub Actions workflow to enable them.",
    );
  }
