...
const githubRepository = /^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/.test(
  releaseConfig.githubRepository || "",
)
  ? releaseConfig.githubRepository
  : null;

const TRUSTED_UPDATE_SOURCES = githubRepository
  ? [
      {
    id: "github",
    ...
    // a different repo on github.com to serve a malicious binary.
        pathPrefix: `/${githubRepository}/releases/download/`,
    redirectHosts: [
      "github.com",
...
function register(getMainWindow, { writeSecretMigration }) {
  ...
  ipcMain.handle("download-and-install-update", async (_, { url, format }) => {
    try {
          ...
          : format === "deb"
            ...
            : format === "pacman"
              ...
              : format === "dmg"
                ? ".dmg"
                : ".AppImage";
      const destPath = path.join(os.tmpdir(), `jetflix-update${ext}`);
      ...
      await new Promise((resolve, reject) => {
        ...
        const doRequest = (reqUrl, redirectDepth = 0) => {
          ...
          const req = lib.get(
            ...
            {
              headers: {
                "User-Agent": "Jetflix-AutoUpdater",
      ...
      if (format === "appimage") {
        ...
        if (currentAppImage) {
          const scriptPath = path.join(os.tmpdir(), "jetflix-update.sh");
  ...
  const fetchImageSecure = (url, resolve, redirectDepth = 0) => {
    ...
    https
      .get(
        url,
        { headers: { "User-Agent": "Jetflix-ReleaseNotes" } },
        (res) => {
          if (
            res.statusCode >= 300 &&
            res.statusCode < 400 &&
